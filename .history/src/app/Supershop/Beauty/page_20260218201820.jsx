...
// Full 15 Gadgets Products JSON Data (আগের মতোই রাখা আছে)
const gadgetsData = [
  {
    id: 1,
...
export default function GadgetsPage() {
  ...
  const [showFilter, setShowFilter] = useState(false); // Filter dropdown toggle
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-white overflow-hidden">
      ...
      {/* Search + Filters Section - Fixed icon position + Filter toggle */}
        ...
        <div className="bg-slate-100 dark:bg-slate-800 rounded-3xl p-8 shadow-2xl">
          <div className="flex flex-col md:flex-row items-center gap-6">
            <div className="relative flex-1 group">
              ...
              />
              <FaSearch className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500 dark:text-gray-300 text-xl group-hover:text-blue-600 transition-colors" />
            </div>
            ...
            <div className="flex flex-wrap gap-4 items-center">
              ...
              {/* Filter Dropdown (toggle করলে দেখা যাবে) */}
              {showFilter && (
                <div className="absolute md:relative top-full md:top-auto left-0 md:left-auto mt-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl shadow-xl p-6 z-50 min-w-[200px]" data-aos="fade-down" data-aos-duration="300">
                    ...
                    {categories.map(cat => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setShowFilter(false); // Close dropdown after select
                        }}
                        className={`w-full text-left px-4 py-2 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900 transition-colors ${filterCategory === cat ? 'bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400 font-semibold' : ''}`}
                      >
                        {cat}
                      </button>
                    ))}
      ...
      <section className="px-4 py-20 max-w-7xl mx-auto" data-aos="fade-up" data-aos-delay="300" data-aos-duration="1500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {paginatedProducts.map((product) => (
            <div 
              key={product.id} 
              className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-2xl hover:shadow-[0_0_40px_rgba(59,130,246,0.4)] hover:scale-105 transition-all duration-500 group relative overflow-hidden" 
              data-aos="zoom-in-up" 
              data-aos-delay="200"
            >
              {/* Discount Badge */}
              ...
              <button className="w-full bg-blue-600 text-white py-4 rounded-2xl flex items-center justify-center gap-4 hover:bg-blue-700 hover:scale-105 transition-all duration-300 group-hover:shadow-xl text-lg font-bold shadow-md">
