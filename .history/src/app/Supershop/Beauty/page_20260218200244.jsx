...
export default function GadgetsPage() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-white overflow-hidden">
      {/* Hero Section */}
      <section className="relative h-[60vh] flex items-center justify-center bg-gradient-to-br from-blue-900 to-purple-900 dark:from-slate-950 dark:to-purple-950 overflow-hidden" data-aos="fade-in" data-aos-duration="1500">
        <div className="text-center z-10">
          <h1 className="text-6xl md:text-8xl font-black text-white drop-shadow-2xl">ShopMart Gadgets</h1>
          <p className="text-2xl text-white/90 mt-6">Discover Premium Tech Essentials</p>
        </div>
      </section>

      {/* Advanced Search Bar and Filters */}
      <section className="px-4 py-10 max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-6 bg-slate-100 dark:bg-slate-800 rounded-3xl shadow-2xl" data-aos="fade-up" data-aos-delay="200">
        <div className="relative flex-1 group">
          <input
            ...
            className="w-full px-8 py-5 bg-white dark:bg-slate-700 border-2 border-slate-300 dark:border-slate-600 rounded-full text-lg font-medium transition-all duration-300 focus:border-blue-600 focus:ring-4 focus:ring-blue-200 dark:focus:ring-blue-900 group-hover:border-blue-400 shadow-md"
          />
          <FaSearch className="absolute left-4 top-5 text-slate-500 dark:text-gray-300 text-xl group-hover:text-blue-600 transition-colors" />
        </div>
      ...
      <section className="px-4 py-20 max-w-7xl mx-auto" data-aos="fade-up" data-aos-delay="300" data-aos-duration="1500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {paginatedProducts.map((product) => (
            <div key={product.id} className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-2xl hover:shadow-[0_0_40px_rgba(59,130,246,0.3)] hover:scale-105 transition-all duration-500 group" data-aos="zoom-in-up" data-aos-delay="200">
              <Image src={product.image} alt={product.name} width={400} height={300} className="rounded-2xl mb-6 object-cover group-hover:rotate-2 transition-transform duration-500" />
              <h3 className="text-2xl font-bold mb-2 group-hover:text-blue-600 transition-colors">{product.name}</h3>
              <p className="text-xl mb-4">${product.price} <span className="line-through text-gray-500">${product.oldPrice}</span> <FaTag className="inline text-red-500 ml-2" /></p>
              <div className="flex items-center mb-6">
                <FaStar className="text-yellow-400 mr-2" /> {product.rating} ({product.reviews} reviews) <FaEye className="ml-4 text-gray-500" />
              </div>
              <button className="w-full bg-blue-600 text-white py-3 rounded-2xl flex items-center justify-center gap-3 hover:bg-blue-700 hover:scale-105 transition-all group-hover:shadow-lg">
        ...
        <div className="flex justify-center mt-12 gap-6">
          {Array.from({ length: totalPages }).map((_, index) => (
            <button
              ...
              className={`px-6 py-3 rounded-2xl shadow-md ${currentPage === index + 1 ? 'bg-blue-600 text-white' : 'bg-slate-200 dark:bg-slate-700 hover:bg-blue-100 dark:hover:bg-blue-900 transition-all'}`}
            >
              {index + 1}
            </button>
          ))}
