...
const ProductCard = ({ product }) => {
  return (
    ...
    >
      ...
      <div className="relative h-[300px] w-full overflow-hidden rounded-[25px] transition-all duration-700 transform group-hover:rotate-x-6 group-hover:scale-95">
        <img 
          src={product.image} 
          alt={product.name} 
      ...
      <div className="mt-5 space-y-2">
        <p className="text-orange-500 text-xs font-bold uppercase tracking-widest">{product.category}</p>
        <h3 className="text-[#2D2D2D] font-bold text-xl truncate text-shadow-premium">
          {product.name}
        </h3>
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-black text-[#2D2D2D]">${product.price}</span>
            {product.oldPrice && (
